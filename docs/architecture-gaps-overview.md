...
this engagement at all. Tier 1.5 (PowerShell Direct guest probe, design-only)
is the only remaining candidate for those gaps — see `docs/architecture.md`.
...
| 2 | Static-memory VMs invisible to memory pressure | **OPEN.** Tier 1's `Hyper-V Dynamic Memory VM` counter simply doesn't exist for these VMs. Tier 2 (`hostmetrics/memory`) would close this but is ruled out for this customer; blocked on the Tier 1.5 go/no-go decision instead. |
...
| 4 | No guest filesystem used % | **OPEN.** Tier 2 (`guest-vm-config.yaml`) would close this but is ruled out for this customer (no in-guest collector, opt-in or otherwise). Blocked on the Tier 1.5 (PowerShell Direct guest probe) go/no-go decision. |
