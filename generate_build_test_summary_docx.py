...
add_table(
    ...
    [
        ["0", "hyperv-scvmm-poller (Windows Service) polling SCVMM", "Central SCVMM console box", "Scaffolded (Phase 1)"],
        ...
        ["1.5", "PowerShell Direct guest probe (VMBus, no guest network/agent)", "Host-initiated", "Design only \u2014 now the only path to gaps #2/#4 (Tier 2 ruled out), blocked on go/no-go"],
        ["1.6", "Windows Event Forwarding", "Centralized collector", "Concept documented"],
...
bullet("Tier 1.5 \u2014 PowerShell Direct guest probe: the host reaches into a guest over VMBus (Invoke-Command -VMId) to run a narrow, specific query \u2014 no guest network stack, no guest firewall rule, no agent installed inside the guest at all. This is fundamentally different from Tier 2: nothing is deployed inside the guest, so it doesn't trigger the customer's \u201cno in-guest collector\u201d constraint. Design-only today, but now the only remaining candidate for gaps #2 and #4.")
bullet("Tier 1.6 \u2014 Windows Event Forwarding: a native Windows mechanism that forwards event logs from hosts/guests to one central collector, without installing a per-VM collector. Config-only, not new code \u2014 closes visibility gaps for event data specifically, distinct from the metric-focused tiers above.")
...
add_table(
    ...
    [
        ["hyperv-scvmm-poller", "Central SCVMM console box", "collect-scvmm-metrics.ps1 + enrich-vm-guest-os.ps1 and their wrappers/scheduled tasks", "Phase 1 \u2014 scaffolded"],
        ["hyperv-host-companion", "Every Hyper-V host, alongside the Splunk OTel Collector", "build-hyperv-vm-disk-map.ps1 + collect-hyperv-vm-disk.ps1 (Phase 2); PowerShell Direct guest probe once Tier 1.5 goes green (Phase 3, not started)", "Phase 2 \u2014 built and tested end-to-end this round"],
    ],
...
add_table(
    ...
    [
        ...
        ["docs/phase3-guest-probe-plan.md", "Design for Tier 1.5 (PowerShell Direct guest probe), not yet implemented"],
...
add_table(
    ...
    [
        ["1", "No VM/host \u201cdown\u201d (power-state) detection \u2014 Perfmon only reports state for running VMs", "Solved", "hyperv_vm_up / hyperv_host_up gauges sourced from SCVMM PowerState/OverallState (Tier 0), tagged with the same vm.name/host.name attributes used everywhere else; being migrated into hyperv-scvmm-poller."],
        ["2", "Static-memory VMs invisible to memory-pressure alerting (Dynamic Memory counter doesn't exist for them)", "Open", "Not a bug \u2014 a coverage gap. Tier 2's hostmetrics/memory scraper would close this, but Tier 2 is ruled out for this customer (no in-guest collector, opt-in or otherwise). Blocked on the Tier 1.5 go/no-go decision instead."],
        ...
        ["4", "No guest filesystem used % visible from the host (architectural limitation)", "Open", "Tier 2 (in-guest OTel Collector) would close this, but this customer has explicitly ruled out deploying any collector inside guest VMs \u2014 not just fleet-wide, opt-in included. Blocked on the Tier 1.5 (PowerShell Direct guest probe) go/no-go decision."],
        ...
        ["7", "~20% of VMs emitting no network series", "Solved", "Root-caused to an incorrect Perfmon counter name (\u201cBytes Total/sec\u201d, which doesn't exist on the Hyper-V network objects) that made the metric 0% functional, not ~80% functional. Corrected to the real counter name (\u201cBytes/sec\u201d) and verified populating in Splunk Observability Cloud."],
        ["8", "guest_os accuracy issues (unknown/untagged VMs, heuristic Linux tagging)", "Solved", "Classification logic (SCVMM OperatingSystem field \u2192 Secure Boot template fallback \u2192 opt-in naming heuristic) writes guest_os as a zero-cost dimension property via the SignalFx metadata API; being migrated into hyperv-scvmm-poller's guest_os loop."],
...
bullet("Tier 1.5 (PowerShell Direct guest probe): design-only, not started. This is now the only remaining path to gaps #2 and #4 for this customer, since it closes the guest-filesystem/memory-pressure gaps without deploying anything inside the guest at all. Blocked on a separate PowerShell Direct POC go/no-go decision (latency at fleet scale, Integration Services coverage, credential-model sign-off).")
...
bullet("hyperv-scvmm-poller (Tier 0) is scaffolded but not yet fully tested end-to-end the way hyperv-host-companion was this round \u2014 next priority for the same live-test treatment.")
...
numbered("Make the PowerShell Direct go/no-go decision to unblock Tier 1.5 design work \u2014 this is now the only remaining path to gaps #2 and #4 for this customer, since Tier 2 has been ruled out.")
